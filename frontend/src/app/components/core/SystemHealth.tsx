...
const STATUS_COLOR = {
  up: 'text-positive', degraded: 'text-yellow-500', down: 'text-critical',
} as const;

const BREAKER_COLOR = {
  closed: 'text-positive', 'half-open': 'text-yellow-500', open: 'text-critical',
} as const;
...
export function SystemHealth() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        // Routed through src/api/ (phase_1 §8) — api.health.get() — rather than a bare
        // fetch. No behavior change: same endpoint, same 15s interval.
  ...
  return (
    <div className="w-[300px] flex-shrink-0 border-l border-line bg-surface-sunken flex flex-col font-mono text-xs text-white">
      ...
      <div className="flex-1 p-4 flex flex-col gap-5 overflow-y-auto custom-scrollbar">
        {error && (
          <div className="flex items-start gap-2 text-critical text-[10px] font-bold uppercase tracking-wide">
        ...
        {health && (
          <>
            <section className="flex flex-col gap-2">
              <div className="text-gray-500 uppercase font-bold tracking-widest text-[10px] border-b border-line-subtle pb-1">
                Dependencies
              </div>
              {Object.values(health.dependencies).map((d) => (
                <div key={d.name} className="flex justify-between items-baseline" title={d.error ?? ''}>
                  ...
                  <span className={`font-bold ${d.status === 'up' ? 'text-positive' : 'text-critical'}`}>
              ...
              {health.queue ? (
                Object.entries(health.queue).map(([state, count]) => (
                  <div key={state} className="flex justify-between items-baseline">
                    ...
                    <span className={`font-bold ${state === 'failed' && count > 0 ? 'text-critical' : 'text-white'}`}>{count}</span>
                  </div>
                ))
              ) : (
            ...
            <div className="mt-auto pt-3 border-t border-line text-[9px] text-gray-600 uppercase tracking-widest">
