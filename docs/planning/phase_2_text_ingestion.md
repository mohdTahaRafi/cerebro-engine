...
**Amended during Phase 2 implementation** — two wiring decisions the original text left open, both corrected against live behavior:

1. **The heading stack persists across pages, not per page.** A document's hierarchy declared on page 11 still governs page 12's content until a new heading appears; resetting per page would blank the path for every page that continues a section without repeating its heading.
2. **A section's path resolves at its LAST line, not its first.** The structural splitter (§5.2) merges small sections up to the token budget, so one section routinely contains several headings. Resolving at the first line returned `"Operating Review"` for a section reading `# Operating Review` / `## Regional Detail` / *paragraphs* — dropping exactly the nesting this feature exists to provide. Caught by the milestone walk, not by unit tests: real ingested chunks visibly contained both heading lines while carrying a single-level path, and every offline chunker test passed throughout because each used a fixture whose sections held one heading apiece. The last line is the heading context governing the section's trailing content (the bulk of it), and is identical to the first line's path for any section containing no headings at all.
...
**Amended during Phase 2 implementation** — the Qdrant-outage step above needs one qualification, found while walking this script literally. A Qdrant connection failure is a *transient* error, so §3.2 correctly hands it to BullMQ's retry policy: 3 attempts at 5 s and 25 s of backoff. If Qdrant is restarted inside that ~35 s window — which is exactly what happens when an operator stops it, uploads, and restarts it promptly — attempt 2 or 3 **succeeds**, and the document ends `ready`, not `failed`. Observed live: the document reported `failed` mid-flight, then reached `{"status":"ready","chunkCount":1}` with `attemptsMade: 3`, and the surviving point was the legitimate result of that retry rather than an orphan. This is correct behavior, not a defect — it is the retry policy doing its job.

Verifying the *atomicity* claim therefore requires holding Qdrant down past all three attempts (~35 s; 75 s was used) so the job reaches a terminal `failed` state first. Done that way the guarantee holds exactly as written: job state `failed` with `attemptsMade: 3`, document `{"status":"failed","error":"fetch failed"}`, and after restarting Qdrant a scroll by that documentId returns **0** points. The transient `error` string surfaced by the Qdrant REST client is the terse `"fetch failed"` (undici's message) rather than a more descriptive connection error.
